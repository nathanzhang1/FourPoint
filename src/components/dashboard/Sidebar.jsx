...
export default function Sidebar() {
    ...
    const { plans } = useContext(UserContentContext);

    const handleHomeClick = () => {
        navigate("/");
    };

    const defaultPlan = plans.find((plan) => plan.defaultplan === "Yes") || plans[0];

    const handleDegreeClick = () => {
        const { id } = defaultPlan;
        if (!id) {
            navigate("/"); // Redirect home if no plans exist
        }
        else {
            navigate(`/plan/${id}`, { state: { ...defaultPlan } });
        }
    }
    ...
    return (
        <div className={styles.sidebar}>
            ...
            <div> </div>
            <div className={styles.tab} onClick={handleHomeClick}>
            ...
            <div className={styles.tab} onClick={handleDegreeClick}>
